  ...
  class RemoteLomakeRepository extends LomakeRepository with Logging {
    ...
    def lomakeAndHakuByApplication(application: ImmutableLegacyApplicationWrapper)(implicit lang: Language.Language): (Option[Lomake], Option[Haku]) = {
      application.hakuOid match {
        case "" => (None, None)
        case hakuOid => (tryFind(hakuOid), tarjontaService.haku(hakuOid, lang).filter(_.published).filter(_.hakukierrosvoimassa))
      }
    }
    ...
    private def tryFind(applicationSystemOid: String): Option[Lomake] = {
      try {
        ...
        case e: ApplicationSystemNotFound =>
          None
        case e: Exception =>
    ...
    def reportCacheStats(): Unit = {
      if (lastReport.plusHours(1).isBeforeNow) {
        ...
        logger.info(s"Reporting hourly cache stats on lomake repository cache: " + repository.asInstanceOf[ApplicationSystemServiceImpl].getCache.stats().toString)
      }
    }

    override def isMaksumuuriKaytossa(oid: String): Boolean = {
      repository.getApplicationSystem(oid).isMaksumuuriKaytossa
    }
  }
}
...
trait LomakeRepository {
  ...
  def lomakeAndHakuByApplication(application: ImmutableLegacyApplicationWrapper)(implicit lang: Language.Language): (Option[Lomake], Option[Haku])
  def applicationPeriodsByOid(applicationSystemId: String)(implicit lang: Language.Language) : List[Hakuaika]
  def isMaksumuuriKaytossa(oid: String): Boolean
}
