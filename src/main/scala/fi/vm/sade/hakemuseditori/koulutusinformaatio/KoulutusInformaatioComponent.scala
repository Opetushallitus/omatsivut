...
trait KoulutusInformaatioComponent {
  ...
  class RemoteKoulutusService() extends KoulutusInformaatioService with JsonFormats with Logging {
    ...
    def opetuspisteet(asId: String, query: String, lang: Language): Option[List[Opetuspiste]] = {
      val (_, _, resultString) = DefaultHttpClient.httpGet(OphUrlProperties.url("koulutusinformaatio-app.lop.search", query))(AppConfig.callerId)
        .param("asId", asId)
    ...
    def opetuspiste(id: String, lang: Language): Option[Opetuspiste] = {
      DefaultHttpClient.httpGet(OphUrlProperties.url("koulutusinformaatio-app.lop", id))(AppConfig.callerId).response.flatMap { resultString =>
    ...
    def koulutukset(asId: String, opetuspisteId: String, baseEducation: Option[String], vocational: String, uiLang: Language): Option[List[Koulutus]] = {
      var request = DefaultHttpClient.httpGet(OphUrlProperties.url("koulutusinformaatio-app.ao.search", asId, opetuspisteId))(AppConfig.callerId)
    ...
    def koulutus(aoId: String, lang: Language): Option[Koulutus] = {
      val request = DefaultHttpClient.httpGet(OphUrlProperties.url("koulutusinformaatio-app.ao", aoId))(AppConfig.callerId)
        .param("lang", lang.toString)
    ...
    def koulutusWithHaku(aoIds: List[String], lang: Language): Option[List[KoulutusInformaatioBasketItem]] = {
      var request = DefaultHttpClient.httpGet(OphUrlProperties.url("koulutusinformaatio-app.basketitems"))(AppConfig.callerId)
        .param("uiLang", lang.toString)
      aoIds.foreach(a =>  {
