...
trait HakemusRepositoryComponent {
  ...
  class HakemusFinder {
    ...
    def getHakemus(hakemusOid: String, fetchTulosForHakemus: ImmutableLegacyApplicationWrapper => Boolean = _ => true)(implicit lang: Language): Option[HakemusInfo] = {
      fetchHakemukset(new Application().setOid(hakemusOid), fetchTulosForHakemus).headOption
    }

    private def fetchHakemukset(query: Application, fetchTulosForHakemus: ImmutableLegacyApplicationWrapper => Boolean)(implicit lang: Language): List[HakemusInfo] = {
