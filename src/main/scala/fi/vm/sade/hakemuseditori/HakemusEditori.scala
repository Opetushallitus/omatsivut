...
trait HakemusEditoriComponent extends ApplicationValidatorComponent
  ...
  trait HakemusEditori {
    ...
    def fetchByHakemusOid(request: HttpServletRequest,
      ...
      val fromHakuApp = hakemusRepository.getHakemus(request, hakemusOid, valintatulosFetchStrategy)
      val fromAtaru = ataruService.findApplications(request, personOid, valintatulosFetchStrategy).find(_.hakemus.oid == hakemusOid)

      if (fromHakuApp.isEmpty) {
        logger.info("fetchByHakemusOid(): Hakemus repository returned no application for given hakemusOid {}.", hakemusOid)
      }
      if (fromAtaru.isEmpty) {
        logger.info("fetchByHakemusOid(): Ataru returned no application for given hakemusOid {}.", hakemusOid)
      }
      fromHakuApp.orElse(fromAtaru).filter(_.hakemus.personOid != personOid).foreach(h => {
        logger.info("fetchByHakemusOid(): Application for hakemusOid {} has personOid {}, different from query oid {}",
          hakemusOid, h.hakemus.personOid, personOid)
      })

      fromHakuApp.orElse(fromAtaru)
    }
