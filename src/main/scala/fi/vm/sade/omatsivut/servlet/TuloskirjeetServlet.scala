  ...
  class TuloskirjeetServlet(val appConfig: AppConfig) extends OmatSivutServletBase {
    ...
    get("/:token/tuloskirje.pdf") {
      (for {
        ...
        hakemusInfo <- Try(hakemusRepository.getHakemus(hakemusOid, false))
        tuloskirje <- Try(hakemusInfo.map(hakemus => tuloskirjeService.fetchTuloskirje(hakemus.hakemus.haku.oid, hakemusOid)))
      } yield {
        tuloskirje match {
