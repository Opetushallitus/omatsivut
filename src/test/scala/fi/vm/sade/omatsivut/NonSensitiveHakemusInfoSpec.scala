...
import fi.vm.sade.hakemuseditori.lomake.domain.{AnswerId, QuestionId, QuestionLeafNode}
  ...
  "NonSensitiveHakemusInfo" should {
    ...
    "return only answers that have questions" in {
      withHakemus(hakemusKorkeakoulutKevat2014Id) { hakemusInfo =>
        ...
        val nonSensitiveInfo = NonSensitiveHakemusInfo.apply(modifiedHakemusInfo, "dummyToken")
    ...
    "always has nonsensitive contact information" in {
      withHakemus(hakemusKorkeakoulutKevat2014Id) { hakemusInfo =>
        val nonSensitiveInfo = NonSensitiveHakemusInfo.apply(hakemusInfo, "dummyToken")
